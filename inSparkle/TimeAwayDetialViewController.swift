//
//  TimeAwayDetialViewController.swift
//  inSparkle
//
...
class TimeAwayDetialViewController: UIViewController {
    ...
    @IBOutlet var listOfDates: UITextView!
    @IBOutlet var alloted: UILabel!
    ...
    override func viewDidLoad() {
        ...
        vacaTimeQuery?.getFirstObjectInBackgroundWithBlock({ (employeeVaca : PFObject?, error : NSError?) in
            if error == nil {
                ...
                self.alloted.text! = String(self.empVaca.issuedHours)
                self.pending.text! = String(self.empVaca.hoursPending) + " (includes current request)"
                self.remaining.text! = String(self.empVaca.hoursLeft) + " (includes pending requests)"
            }
        })
    }
